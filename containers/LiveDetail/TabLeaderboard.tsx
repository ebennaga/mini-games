...
import dataLeaderboard from './dummy';
    ...
    return (
        <TabPanel index={index} value={value}>
            <Box sx={{ display: 'flex', justifyContent: 'center', m: '20px', alignItems: 'center', flexDirection: 'column' }}>
                {dataLeaderboard.length > 0 && <LeaderboardPodium dataLeaderboard={dataLeaderboard} />}
                <Box sx={{ mt: '20px', width: '100%' }}>
                    {dataLeaderboard.length > 0 && <TableRank dataLeaderboard={dataLeaderboard} />}
                </Box>
                {/* <Box
                    sx={{
                        position: 'relative',
